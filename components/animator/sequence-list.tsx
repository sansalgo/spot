...
export function SequenceList() {
  ...
  return (
    <div className="w-40 border-r border-border flex flex-col overflow-hidden">
      <div className="px-3 pt-3 text-[9px] text-muted-foreground tracking-widest shrink-0">SEQUENCES</div>

      <ScrollArea className="flex-1 min-h-0 px-3 py-1.5">
        <div className="flex flex-col gap-1.5 pb-1">
          {sequences.map((seq, si) => (
            ...
            >
              <div className="flex-1 min-w-0">
                ...
                <div className="text-[9px] text-muted-foreground mt-0.5">{seq.frames.length}f</div>
              </div>

              <div className="flex items-center gap-0.5 ml-1 shrink-0">
                {editingIndex === si ? (
                  <button
                    onClick={(e) => { e.stopPropagation(); commitRename(si); }}
                  ...
                  <button
                    onClick={(e) => { e.stopPropagation(); startRename(si, seq.name); }}
                ...
                {sequences.length > 1 && (
                  <button
                    onClick={(e) => { e.stopPropagation(); deleteSequence(si); }}
                    className="text-muted-foreground hover:text-foreground transition-colors"
                  >
                    <X size={11} />
                  </button>
          ...
          <Button
            ...
            variant="ghost"
            size="sm"
            className="mt-1 w-full text-[11px] border border-dashed border-border font-mono"
          >
            + ADD SEQ
          </Button>
        </div>
      ...
      <div className="p-3 border-t border-border shrink-0">
        <div className="text-[9px] text-muted-foreground tracking-widest mb-2">PREVIEW ALL</div>
        <SpotPlayer
